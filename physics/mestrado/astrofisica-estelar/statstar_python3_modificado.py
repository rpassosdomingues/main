...
#
      ...
      for ic in range(0,istop+1):
          ...
          if (np.abs(dlPdlT[i]) > dlPlim):
              ...
              clim = ' '
          s='{0:7.2E} {1:7.2E} {2:7.2E} {3:7.2E} {4:7.2E} {5:7.2E} {6:7.2E} {7:6.2E} {8:5.1f}\n'.format(r[i], Qm, L_r[i], T[i], P[i], rho[i], kappa[i], epslon[i], dlPdlT[i])
          f.write(s)
...
#
    M05,L05,T05 = (0.5, 0.02150, 2331.45)
    M1,L1,T1 = (1.0, 0.86071, 5500.20)
    M2,L2,T2 = (2.0, 22.61200, 11218.40)
    M3,L3,T3 = (3.0, 116.57999, 15007.30)
    M4,L4,T4 = (4.0, 341.09998, 17904.00)
    M5,L5,T5 = (5.0, 747.92000, 20302.00)
    M6,L6,T6 = (6.0, 1375.34998, 22310.00)
    M7,L7,T7 = (7.0, 2260.19995, 24074.00)
    M8,L8,T8 = (8.0, 3421.51978, 25613.60)
    M9,L9,T9 = (9.0, 4880.79980, 27061.20)
    M10,L10,T10 = (10.0, 6641.59961, 28263.60)

    getinp=1  # read in input
    if (getinp == 1):
        Msolar=M1#float(input(' Enter the mass of the star (in solar units):'))
        Lsolar=L1#float(input(' Enter the luminosity of the star (in solar units):'))
        Te=T1#float(input(' Enter the effective temperature of the star (in K):'))
        Y=-1.0
        while (Y < 0.0):
            X=0.7#float(input(' Enter the mass fraction of hydrogen (X):'))
            Z=0.008#float(input(' Enter the mass fraction of metals (Z):'))
            Y = 1.e0 - X - Z
            if Y < 0:
    ...
    [[float(token) for token in line.split()]  
        for line in open("starmodl_py.dat") if line.strip()],
     columns = ["r", "Qm", "L", "T", "P", "rho", "kappa", "epslon", "dlPdlT"]
    )

    r = np.array(dados['r'])  # raio
    
    Qm = np.array(dados['Qm']) # (1) Mass is listed as Qm = 1.0 - M_r / Mtot, where Mtot = 1.989e+33
    
    M = mass(Qm) # massa
    L = np.array(dados['L'])  # luminosidade
    T = np.array(dados['T'])  # temperatura
    P = np.array(dados['P'])  # pressao
    
    rho = np.array(dados['rho'])  # densidade
    kappa = np.array(dados['kappa'])  # opacidade
    epslon = np.array(dados['epslon'])  # taxa de producao de energia
    dlPdlT = np.array(dados['dlPdlT'])  # transicao zonas: convectiva e radiativa

    # Normalizando as escalas
    
    r_norm = normalize(r)
    
    Qm_norm = normalize(Qm)
    M_norm = normalize(M)
    ...
    kappa_norm = normalize(kappa)
    epslon_norm = normalize(epslon)
    dlPdlT_norm = normalize(dlPdlT)
    
    r_max = np.amax(r)
    T_max = np.amax(T)
    P_max = np.amax(P)
    rho_max = np.amax(rho)
    epslon_max = np.amax(epslon)
    
    print('=' * 50)
    print('Constantes de Normalização: Escala do Gráfico')
    print('-' * 50)
    print(" r_norm = {0:12.4f} \n T_norm = {1:12.4f} \n P_norm = {2:12.4f} \n rho_norm = {3:12.4f}\n epslon_norm = {2:12.4f} \n".format(r_max,T_max,P_max,rho_max,epslon_max))
    print('=' * 50)
    ...
    #print('Lsun = {0} [erg/s]'.format(Lsun))
    #print('Lsolar = {0} [Lo]'.format(Lsolar))
    #print('Ltot = {0} [erg/s]'.format(Ltot))
    
    #L50 = 0.50 * Ltot
    #L99 = 0.99 * Ltot
    
    #print('L50 = {0} [erg/s]'.format(L50))
    #print('L99 = {0} [erg/s]'.format(L99))
    
    # Gráfico
    fig, ax = plt.subplots()
    
    plt.plot(r_norm, T_norm, c='red', label='Temperatura', linewidth=2)
    plt.plot(r_norm, P_norm, c='purple', label='Pressão', linewidth=2)
    plt.plot(r_norm, rho_norm, c='yellow', label='Densidade', linewidth=2)
    plt.plot(r_norm, epslon_norm, c='blue',label='Produção de Energia', linewidth=2)
    plt.legend()
    
    plt.title("$1.0 M_\odot$")
    
    plt.xlabel("Raio (normalizado)")
    plt.ylabel("T, P, $p$, $E$ (normalizados)")
